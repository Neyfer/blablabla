$(document).ready(function(){
    ...
    if(document.URL.includes("dashboard.php")){
        $("#dash").addClass("active");
    }

    if(document.URL.includes("info.php")){
        $("#coder-dash").addClass("active");
    }
    ...
    if(document.URL.includes("ingresar_notas.php")){
        ...
        function update_notas(){
            ...
            $.ajax({
                ...
                //INHABILITAR EL CAMBIO NATURAL DE VALORES UNA VEZ LA NOTA SE ESCUENTRA EN LA BASE DE DATOS Y PONER LA LETRA ROJA SI LA NOTA ES MENOR A 70
        ...
        function editar(){
            $(".nota-div").each(function(){
                if($(this).children('.notaaa')[0].value != ""){
                    ...
                    $(this).dblclick(()=>{
                        ...
                        }else if($(this).hasClass("recuperacion-div-c")){
                            parcial_string = "recuperacion_ciclo";
                        }else if($(this).hasClass("recuperacion-btp1")){
                            parcial_string = "recuperacion1";
                        }else if($(this).hasClass("recuperacion-btp2")){
                            parcial_string = "recuperacion2";
                        }
    
                        console.log(parcial_string);
        ...
        function get_subjects(){
            ...
            if(grade[0].value == "7" || grade[0].value == "8" || grade[0].value == "9" || grade[0].value == "10"){
            ...
            }else if(grade[0].value == "11" || grade[0].value == "12"){
                // SE ESTABLECE LA VARIABLE BTP A TRUE PARA CAMBIAR LA APARIENCIA DE LA TABLA
                btp = true;

                let grade2;

                //ACTIVAR EL SELECTOR DE SEMESTRE PARA DETEREMINAR LAS CLASES CURSADAS EN BTP
                $("#semester-selector").show();

                //VER QUE SEMESTRE ESTA BTP
                if(grade[0].value == "11" && parcial[0].value == "2"){
                    grade2 = "11.5";
        ...
        btn_save.click(function(e){
            ...
            function primer_semestre(){
                ...
                let recuperacion1 = document.querySelectorAll(".recuperacion");

                p_parcial_counter = 0;
                s_parcial_counter = 0;
                recuperacion_counter1 = 0;
                ...
                let subject = document.getElementById("subject").value;
                    ...
                    //Checar los reords de recuperacion 1er semestre
                    recuperacion1.forEach(name=>{
                        if(name.value != ""){
                            recuperacion_counter1++;
                        }

                        console.log(recuperacion_counter1);
                    })
                ...
                //Subir los datos del primer parcial
                    ...
                    //SUBIR RECUPERACION BTP 1ER SEMESTRE
                    if(parseInt(grade[0].value) > 10){
                        if(recuperacion_counter1 > 0){
                            for(i = 0; i < recuperacion1.length; i++){
                                if(recuperacion1.value != ""){
                                    if(recuperacion1[i].hasAttribute("disabled") == false){
                                    $.ajax({
                                        url: '../php/server.php?recuperacion1',
                                        type: 'post',
                                        data: {'nota': recuperacion1[i].value, 'subject': subject, 'name': student_name[i].textContent, 'sudent_id': student_id[i].value, 'grade': grade[0].value},
                            ...
                            console.log("NO RECORDS FOR FOURTH REC1")
                        }
                    }
            }
            ...
            function segundo_semestre(){
                ...
                let recuperacion = document.querySelectorAll(".recuperacion");
                let recuperacion_counter = 0;
                    ...
                    //checar si s eingresaron datos
                     //Checar los records del tercer parcial
                        ...
                        c_parcial.forEach(name=>{
                         ...
                         //Checar los records recuperacion
                        recuperacion.forEach(name=>{
                            if(name.value != ""){
                                recuperacion_counter++;
                            }

                            console.log(recuperacion_counter);
                        })
                ...
                //SUBIR LAS NOTAS DE RECUPERACION DE CICLO

                if(parseInt(grade[0].value) < 11){
                    console.log("started")
                    if(recuperacion_counter > 0){
                        console.log("more than 0")
                        for(i = 0; i < recuperacion.length; i++){
                            if(recuperacion[i].value != ""){
                                if(recuperacion[i].hasAttribute("disabled") == false){
                                $.ajax({
                                    url: '../php/server.php?recuperacion_ciclo',
                                    type: 'post',
                                    data: {'nota': recuperacion[i].value, 'subject': subject, 'name': student_name[i].textContent, 'sudent_id': student_id[i].value, 'grade': grade[0].value},
                        ...
                        console.log("NO RECORDS FOR REC S")
                    } //RECUPERACION BTP SEGUNDO SEMESTRE
                }else{
                    if(recuperacion_counter > 0){
                        for(i = 0; i < recuperacion.length; i++){
                            if(recuperacion.value != ""){
                                if(recuperacion[i].hasAttribute("disabled") == false){
                                $.ajax({
                                    url: '../php/server.php?recuperacion2',
                                    type: 'post',
                                    data: {'nota': recuperacion[i].value, 'subject': subject, 'name': student_name[i].textContent, 'sudent_id': student_id[i].value, 'grade': grade[0].value},
                        ...
                        console.log("NO RECORDS FOR REC2")
                    }
                }
    ...
    if(document.URL.includes("Materias.php")){
        ...
        $(".edit_subject").each(function(){
            $(this).click(()=>{
                $("#edit_grade")[0].value = $("#grade-s")[0].value;
                fetch(`../php/server.php?get_subject_info=${$(this).attr("id")}`).then(res => res.json()).then(data=>{
                    $(".id")[0].value = data.id;
                    $("#edit_prev_sub")[0].value = data.subject;
                    $("#edit_name")[0].value = data.subject;
                    $("#edit_teacher")[0].value = data.teacher;
                    $("#edit_horas")[0].value = data.hours;
                })
            })
        })
        ...
        $("#edit-subject").click(function(e){
            e.preventDefault();
            $.ajax({
                url: `../php/server.php?edit_subject`,
                type: "post",
        ...
        $(".delete_subject").click(function(){
            $("#delete_id")[0].value = this.id;
            $("#grade")[0].value = $("#grade-s")[0].value;
        })

        $("#delete_subject_button").click(function(e){
            e.preventDefault();
            $.ajax({
                url: `../php/server.php?delete_subject`,
                type: "post",
                data: $("#delete_subject_form").serialize(),
                success: function(res){
                    //ACTUALIAR LA TABLA LUEGO DE ELIMINAR LA ASIGNATURA
                    fetch(`../php/server.php?get_subjects_back=${$("#grade")[0].value}`).then(res => res.text()).then(data=>{
                        console.log(data)
                        $(".materias-t").html(data);
                    })
                }
            })
